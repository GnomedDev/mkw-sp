...
void ThumbnailManager::nextDir() {
    ...
    for (; !m_dir && m_courseId < 0x20; m_courseId++) {
        ...
        swprintf(path.data(), path.size(), L"/mkw-spc/thumbnails/inputs/%u", m_courseId);
...
void ThumbnailManager::capture() {
    ...
    swprintf(path.data(), path.size(), L"/mkw-spc/thumbnails/outputs");
    if (!Storage::CreateDir(path.data(), true)) {
        return;
    }

    swprintf(path.data(), path.size(), L"/mkw-spc/thumbnails/outputs/%u", m_courseId - 1);
    if (!Storage::CreateDir(path.data(), true)) {
        return;
    }

    swprintf(path.data(), path.size(), L"/mkw-spc/thumbnails/outputs/%u/%s.xfb", m_courseId - 1,
            m_name->data());
...
FixedString<64> ThumbnailManager::path() {
    FixedString<64> path{};
    auto len = snprintf(path.m_buf.data(), 64, "/mkw-spc/thumbnails/inputs/%u/%s", m_courseId - 1,
            m_name->data());
    assert(len > 0);
